 ...
 * Utilisée par toutes les pages publiques (index, dashboard, payment) pour afficher
 * les prix barrés + prix promo si une promotion est en cours.
 *
 * GET /api/quiz/prices →
 *   {
 *     prices: [
 *       { type_concours, prix_normal, prix_promo (nullable), promo (objet|null) }
 *     ]
 *   }
 */
...
export default async function handler(req) {
  ...
  try {
    ...
    // 2. Promotions actives (depuis correction_requests, marqueur ifl_promo)
    ...
    if (promoRows) {
      for (const row of promoRows) {
        try {
          ...
          // Garde la plus récente promo active par type
    ...
    const buildEntry = (type, prix_normal) => {
      ...
      return {
        type_concours: type,
        prix_normal,
        prix: promo ? promo.prix_promo : prix_normal, // prix effectif (pour compat)
    ...
    return new Response(JSON.stringify({
      prices: [
        buildEntry('direct', directPrix),
        buildEntry('professionnel', profPrix)
      ]
    }), {
      status: 200,
      headers: {
