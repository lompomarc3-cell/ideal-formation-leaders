  ...
  return (
    <>
      ...
      <div className="min-h-screen" style={{ background: '#FFF8F0' }}>
        ...
        <a
          href={`https://wa.me/22676223962?text=${encodeURIComponent("Bonjour IFL, j'ai besoin d'aide pour choisir ma spécialité")}`}
          onClick={(e) => {
            try { e.preventDefault() } catch {}
            const url = `https://wa.me/22676223962?text=${encodeURIComponent("Bonjour IFL, j'ai besoin d'aide pour choisir ma spécialité")}`
            try {
              const isMobile = /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent || '')
              const isPWA = window.matchMedia && window.matchMedia('(display-mode: standalone)').matches
              if (isMobile || isPWA) { window.location.href = url } else { window.open(url, '_blank', 'noopener,noreferrer') }
